...
k = 1;
s = 0;
gamma_T = [1,4,8;2,3,6;1,4,8;2,3,6]; %local node number for Dirichlet BC
T_bar = [0,1,0,1]; %temp for Dirichlet BC
...
figure('Name','Problem 1 Temperature Field','NumberTitle','off')
surf(Xq,Yq,zeros(size(Xq)),Tq)
view(2)
shading interp
a=colorbar;
title('Problem 1 Temperature Field')
xlabel('Position in meters')
ylabel('Position in meters')
a.Label.String = 'Temperature';

figure('Name','Problem 1 Heat Flux','NumberTitle','off')
quiver(x_qp,y_qp,q(1,:),q(2,:))
hold on
scatter(x_qp,y_qp)
title('Problem 1 Heat Flux')
xlabel('Position in meters')
ylabel('Position in meters')
hold off
